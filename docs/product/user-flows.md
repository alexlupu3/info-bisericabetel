...
- Steps:
  ...
  - To delete a group: admin clicks "Șterge grup" on the group header; all items in that group are moved to the ungrouped section before the group is removed
...
- Failure or edge states:
  - A group still has items when deleted (handled: items auto-migrate to ungrouped)
