...
- First-pass translations are automatically generated by Claude Haiku when content items or groups are created or updated (if `ANTHROPIC_API_KEY` is configured); see [ADR-010](ADR-010-ai-auto-translation-fire-and-forget.md) for the fire-and-forget job decision.
