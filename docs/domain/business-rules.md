...
- BR-013: Chronological ordering within a group uses `data.startDate` as the sort key (YYYY-MM-DD string, ascending). The sort is stable — items with the same startDate preserve their prior relative order. Items without a startDate are always placed after all dated items. When the sort button ("Sortează cronologic") is clicked on a group whose items are all undated, the action is a silent no-op and the order is unchanged. When a new item is created inside a group, the group is immediately re-sorted chronologically: the new item is treated as if appended last before sorting, so items with an equal startDate that already existed in the group remain before the new item. The resulting order is persisted via `PUT /api/admin/content/order`. Manual drag-and-drop ordering is unaffected and remains available as an alternative.
...
- BR-013 added: 2026-05-13 (group chronological sort feature, FR-046)
